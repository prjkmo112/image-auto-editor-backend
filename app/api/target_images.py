...
async def create_target_image(
    ...
    try:
        # make upload dir
        upload_dir = Path(os.getenv("SAVED_IMG_DIR")) / "eimg"
...
async def get_target_images_list(
    ...
    active_only: bool = Query(True, description="활성 이미지만 조회"),
    db: AsyncSession = Depends(get_db),
...
async def delete_target_image(
    image: TargetImageResponse, db: AsyncSession = Depends(get_db)
):
    """이미지 삭제"""
    try:
        # 파일 삭제
